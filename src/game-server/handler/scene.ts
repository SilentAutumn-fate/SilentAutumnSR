...
async function loadScene(player: NetSession, dataModule: DataService, entry_id: number, teleport_id?: number[]) {
    ...
    const sceneInfo: starrail.SceneInfo = new starrail.SceneInfo({
        ...
        entityGroupList: []
        // 4.5.51: JNCGAINGMMM 字段已被移除
    });

    // Load player entity
    const playerGroup: starrail.SceneEntityGroupInfo = new starrail.SceneEntityGroupInfo({
        state: 1,
    ...
    sceneInfo.entityGroupList.push(playerGroup);
    ...
    for (const [groupId, group] of Object.entries(groupConfig.group_items)) {
        const groupInfo: starrail.SceneEntityGroupInfo = new starrail.SceneEntityGroupInfo({
            state: 1,
        ...
        sceneInfo.entityGroupList.push(groupInfo);
    }
    return sceneInfo;
}
